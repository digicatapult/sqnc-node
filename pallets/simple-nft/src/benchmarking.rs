...
fn add_nfts<T: Config>(r: u32) -> Result<(), &'static str> {
    ...
    let outputs: Vec<_> = (0..r).map(|_| (owner.clone(), T::TokenMetadata::default())).collect();
...
fn mk_outputs<T: Config>(o: u32) -> Result<Vec<(T::AccountId, T::TokenMetadata)>, &'static str> {
    let owner: T::AccountId = account("owner", 0, SEED);
    let outputs = (0..o)
        .map(|_| (owner.clone(), T::TokenMetadata::default()))
        .collect::<Vec<_>>();
