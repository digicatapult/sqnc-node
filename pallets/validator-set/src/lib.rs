...
pub mod pallet {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn remove_validator(origin: OriginFor<T>, validator_id: T::ValidatorId) -> DispatchResult {
            T::AddRemoveOrigin::ensure_origin(origin)?;
...
impl<T: Config> EstimateNextSessionRotation<BlockNumberFor<T>> for Pallet<T> {
    ...
    fn estimate_current_session_progress(_now: BlockNumberFor<T>) -> (Option<sp_runtime::Permill>, sp_weights::Weight) {
        (None, Zero::zero())
    }

    fn estimate_next_session_rotation(_now: BlockNumberFor<T>) -> (Option<BlockNumberFor<T>>, sp_weights::Weight) {
        (None, Zero::zero())
    }
}
    ...
    ReportOffence<T::AccountId, (T::ValidatorId, T::ValidatorId), O> for Pallet<T>
{
    ...
    fn is_known_offence(_offenders: &[(T::ValidatorId, T::ValidatorId)], _time_slot: &O::TimeSlot) -> bool {
        false
    }
}
