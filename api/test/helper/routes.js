...
const getLastTokenId = async (context) => {
  ...
  const lastTokenId = await context.api.query.simpleNFTPallet.lastToken()
  return lastTokenId ? parseInt(lastTokenId, 10) : -1
}

const getToken = async (context, tokenId) => {
  await context.api.isReady
  const token = await context.api.query.simpleNFTPallet.tokensById(tokenId)
  return token.toJSON()
}
...
const runProcess = async (context, process, inputs, outputs) => {
  ...
  return new Promise((resolve, reject) => {
    let unsub = null
    context.api.tx.simpleNFTPallet
      ...
      .signAndSend(alice, (result) => {
        if (result.status.isInBlock) {
          ...
          if (errors.length > 0) {
            reject('ExtrinsicFailed error in simpleNFTPallet')
          }
